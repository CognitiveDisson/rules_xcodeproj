...
""")
        ...
        library_files = []
        for file in objc.library.to_list():
            if sets.contains(avoid_libraries, file):
                continue
            library_files.append(file)
            libraries.append(file_path_to_dto(file_path(file)))
        imported_libraries = [
            file_path_to_dto(file_path(file))
        ...
        for input in cc_info.linking_context.linker_inputs.to_list():
            ...
            for library in input.libraries:
                if sets.contains(avoid_libraries, library):
                    continue
                library_files.append(library.static_library)
                libraries.append(
...
def _get_static_libraries(linker_inputs):
    ...
    if not top_level_values:
        print(_extract_top_level_values(ctx = None, cc_info = linker_inputs._cc_info, objc = linker_inputs._objc, avoid_linker_inputs = None))
        fail("Xcode target requires `ObjcProvider` or `CcInfo`")
    return top_level_values.library_files
...
def _to_dto(linker_inputs):
    ...
    ret = {}
    set_if_true(ret, "dynamic_frameworks", top_level_values.dynamic_frameworks)
    set_if_true(ret, "linkopts", top_level_values.linkopts)
    set_if_true(
        ret,
        "static_libraries",
        top_level_values.libraries + top_level_values.imported_libraries,
    )
    set_if_true(ret, "static_frameworks", top_level_values.static_frameworks)
